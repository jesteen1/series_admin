...
const EpisodeCard = ({
    ...
    if (selectseries !== series) return null;

    return (
        <div className="w-80 flex flex-col rounded-xl overflow-hidden bg-zinc-900 border border-zinc-800 shadow-lg hover:shadow-xl transition-all duration-300 group hover:scale-105 cursor-pointer">
            ...
            <div className="relative p-4 bg-gradient-to-b from-zinc-900 to-zinc-950 flex-1">
                <h3 className="text-lg font-bold w-full text-white  group-hover:text-red-500 transition-colors duration-300" title={episodeName}>
                    {episodeName || "Unnamed Episode"}
                </h3>
                <p id="descri" className="absolute bottom-full left-0 w-full bg-zinc-900/95 backdrop-blur-sm p-4 text-[11px] font-bold text-zinc-400 leading-relaxed tracking-wide uppercase transform translate-y-4 opacity-0 group-hover:translate-y-0 group-hover:opacity-100 transition-all duration-300 pointer-events-none border-t border-white/10 shadow-lg empty:hidden">
                    {descri}
                </p>
                {createdAt && (
