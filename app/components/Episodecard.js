...
const EpisodeCard = ({
    ...
    return (
        ...
        {selectseries==series?<div className="w-80 rounded-xl overflow-hidden bg-zinc-900 border border-zinc-800 shadow-lg hover:shadow-xl transition-all duration-300 group hover:scale-105 cursor-pointer">
            ...
            <div className="text-white flex gap-1 p-5 justify-between">
                <div className="  ">
                    <button data-id={id} onClick={(e)=>editdata(e.target.dataset.id)} className="bg-blue-500 p-5 cursor-pointer">Edit</button>
                </div>
                <div className="">
                     <button data-id={id} onClick={(e)=>deletedata(e.target.dataset.id)} className="bg-red-500 p-5 cursor-pointer" >Delete</button>
                </div>
            </div>
        </div>
:null}
